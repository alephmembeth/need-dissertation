...
\begin{itemize}[label=,leftmargin=2em,itemindent=-2em]
   ...
   \item Bauer, Alexander Max, Jan Romann, Mark Siebel und Stefan Traub (2023): \enquote{Winter is Coming. How Laypeople Think About Different Kinds of Needs}. In: \textit{PLOS ONE} 18 (11), e0294572.
...
\enquote{Die gesamte Philosophie ist also einem Baume vergleichbar, dessen Wurzeln die Metaphysik, dessen Stamm die Physik und dessen Zweige alle übrigen Wissenschaften sind, die sich auf drei hauptsächliche zurückführen lassen, nämlich auf die Medizin, die Mechanik und die Ethik. [$\cdots$] So wie man nun weder von den Wurzeln noch vom Stamm der Bäume die Früchte pflückt, sondern nur von ihren Zweigen, so hängt auch der hauptsächliche Nutzen der Philosophie von denjenigen ihrer Teile ab, die man erst zu allerletzt lernen kann.} \textcolor{gray}{(Descartes 1992, S. XLII)}
   \vfill
   \begin{center}
      \frame{\includegraphics[width=0.7\linewidth]{figures/slides_tree.jpg}}\\
...
\begin{multicols}{2}
   \begin{center}
      \frame{\includegraphics[width=0.7\linewidth]{figures/slides_lecture_1.jpg}}\\
      \textcolor{gray}{Desaguliers 1719}
   \end{center}
   \vfill
   \begin{center}
      \frame{\includegraphics[width=0.7\linewidth]{figures/slides_lecture_2.jpg}}\\
      \textcolor{gray}{Desaguliers 1719}
   \end{center}
\end{multicols}
\end{frame}
...
\begin{frame}{\vspace*{10mm}1\hspace*{1em}Empirische Forschung und normative Theorie}
\begin{center}
   \frame{\includegraphics[width=0.8\linewidth]{figures/slides_ngram.png}}\\
...
\begin{frame}{\vspace*{10mm}1\hspace*{1em}Empirische Forschung und normative Theorie}
\begin{center}
   \frame{\includegraphics[width=0.8\linewidth]{figures/slides_xphi.png}}\\
...
\note{
   \begin{itemize}
      \item Verantwortlichkeit signifikant höher (zweiseitige $t$-Tests)
      \item Kein Unterschied ob verantwortlich für Bedarf oder Leistung (zweiseitige $t$-Tests)
   \end{itemize}
}
\end{frame}
...
\note{
   \begin{itemize}
      ...
      \item Gestrichelte Linie: Produktivitätsanteil Person A
      \item Höher als der Produktivitätsanteil und niedriger als der Bedarfsanteil (einseitige $t$-Tests)
      \item Geringer wenn verantwortlich, außer bei Fall 1 des Bedarfsszenarios (einseitige $t$-Tests)
   \end{itemize}
}
\end{frame}
...
\begin{center}
   Anteil Bedarf $=\frac{\nu_{A}}{N}$\\[1em]
   Anteil Produktivität $=\frac{\phi_{A}}{\Phi}$\\
   \begin{equation*}
      \textsf{Abweichung}=\left\{\begin{array}{ll}\frac{\textsf{Anteil Holzscheite}-0,5}{\textsf{Anteil Bedarf}-0,5} & \textsf{(Bedarfsszenario)}\\\\\frac{0,5-\textsf{Anteil Holzscheite}}{0,5-\textsf{Anteil Produktivität}} & \textsf{(Produktivitätsszenario)}\end{array}\right.
   \end{equation*}
\end{center}
\note{
   \begin{itemize}
      \item Bedarfsszenario: $0$ wenn Person A bekommt, was sie produziert hat ($<0$ wenn sie weniger bekommt); $1$ wenn sie bekommt, was sie benötigt ($>1$ wenn sie mehr bekommt)
      \item Produktivitätsszenario: $0$ wenn Person A bekommt, was sie benötigt ($<0$ wenn sie weniger bekommt); $1$ wenn sie bekommt, was sie produziert hat ($>1$ wenn sie mehr bekommt)
      \item Anteil Bedarf Person A im Bedarfsszenario: $0,6$ (Fall 1) bis $0,86$ (Fall 5)
      \item Anteil Produktivität Person A im Produktivitätsszenario: $0,14$ (Fall 5) bis $0,40$ (Fall 1)
   \end{itemize}
}
\end{frame}
...
\note{
   \begin{itemize}
         \item Nettoaufteilungsprinzip: Person A erhält Bedarfsanteil zuzüglich oder abzüglich des Überschusses oder Defizits
         \item Signifikant mehr Gleichverteilung in Verantwortlichkeitsgruppe ($\chi^2$-Tests)
         \item Tauschprinzip: Person A erhält mehr als die Hälfte, trotz gleichem Bedarfsanteil und geringerer Produktivität
         \item Signifikant weniger Gleichverteilung in Verantwortlichkeitsgruppe ($\chi^2$-Tests)
   \end{itemize}
}
\end{frame}
