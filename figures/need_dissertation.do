...
preserve
   ...
   twoway (scatter meanjustice units if treatment==0, mcolor(black) msize(medium) msymbol(diamond)) ///
      (rcap ci_high ci_low units if treatment==0, lcolor(black)) ///
      (scatter meanjustice units if treatment==1, mcolor(gs10) msize(medium) msymbol(square)) ///
      (rcap ci_high ci_low units if treatment==1, lcolor(gs10)) ///
      (function y=1-exp(-((lambdat1_0)*x)^(kt1_0)), range(0 1000) lcolor(black)) ///
      (function y=1-exp(-((lambdat1_0+lambdat1_1)*x)^(kt1_0+kt1_1)), range(1000 2000) lcolor(black)) ///
      (function y=1-exp(-((lambdat2)*x)^(kt2)), range(0 2000) lcolor(gs10) lpattern(dash)), ///
      title("") ///
      ytitle("Einschätzung") ///
      xtitle("Wohnraum") ///
      xlabel(0 "0" 500 "500" 1000 "1.000" 1500 "1.500" 2000 "2.000", labsize(small) angle(forty_five)) ///
...
preserve
   ...
   collapse (mean) meanjustice=relative_justice ///
      (sd) sdjustice=relative_justice ///
      (count) n=relative_justice, ///
      by(treatment units)
   ...
   twoway (rcap ci_high ci_low units if treatment==0, lcolor(black)) ///
      ...
      ytitle("Einschätzung") ///
      xtitle("Wohnraum") ///
      xlabel(0 "0, 200" 200 "200, 400" 400 "400, 600" 600 "600, 800" 800 "800, 1.000" 1000 "1.000, 1.200" 1200 "1.200, 1.400" 1400 "1.400, 1.600" 1600 "1.600, 1.800" 1800 "1.800, 2.000", labsize(small) angle(forty_five)) ///
...
preserve
   ...
   twoway (bar meanj tnr if treatment==0, fcolor(gs10) lcolor(black) lwidth(medium) barwidth(0.9)) ///
      ...
      xlabel(0 `""Bedarf," "Niedrige" "Verantw.""' 1 `""Bedarf," "Hohe" "Verantw.""' 2.25 `""Produktivität," "Niedrige" "Verantw.""' 3.25 `""Produktivität," "Hohe" "Verantw.""', labsize(small)) ///
...
preserve
   ...
   twoway (bar meanj tnr if treatment==0, fcolor(gs10) lcolor(black) lwidth(medium) barwidth(0.9)) ///
      ...
      ylabel(0 (0.1) 0.7, labsize(medlarge)) ///
      ...
      legend(off) ///
      graphregion(color(white)) ///
      saving(figure_6_a, replace)
   graph export "figure_6_a.pdf", as(pdf) replace
restore
...
preserve
   ...
   collapse (mean) meanj=deviation_a ///
      (sd) sdj=deviation_a ///
      (count) n=deviation_a, ///
      by(scenario treatment)
   ...
   twoway (rcap ci_high ci_low tnr, lcolor(black)) ///
      ...
      legend(off) ///
      graphregion(color(white)) ///
      saving(figure_6_b, replace)
   graph export "figure_6_b.pdf", as(pdf) replace
restore
...
preserve
   ...
   collapse (mean) meanshare=share_a ///
      (sd) sdj=share_a ///
      (count) n=share_a, ///
      by(case treatment)
   ...
   generate rescase=(case-1)*2+case if treatment==0
   ...
   twoway (bar meanshare rescase if treatment==0, fcolor(gs10) lcolor(black) lwidth(medium)) ///
      ...
      legend(off) ///
      graphregion(color(white)) ///
      saving(figure_6_c, replace)
   graph export "figure_6_c.pdf", as(pdf) replace
restore
...
preserve
   ...
   twoway (rcap ci_high ci_low rescase, lcolor(black)) ///
      ...
      legend(off) ///
      graphregion(color(white)) ///
      saving(figure_6_d, replace)
   graph export "figure_6_d.pdf", as(pdf) replace
restore
...
preserve
   ...
   twoway (bar meanshare rescase if treatment==0, fcolor(gs10) lcolor(black) lwidth(medium)) ///
      ...
      legend(off) ///
      graphregion(color(white)) ///
      saving(figure_6_e, replace)
   graph export "figure_6_e.pdf", as(pdf) replace
restore
...
preserve
   ...
   twoway (rcap ci_high ci_low rescase, lcolor(black)) ///
      ...
      legend(off) ///
      graphregion(color(white)) ///
      saving(figure_6_f, replace)
   graph export "figure_6_f.pdf", as(pdf) replace
restore
...
preserve
   ...
   xtreg deviation_a ///
      ...
      criteria_likert_need ///
      criteria_likert_productivity ///
      criteria_likert_equality accountjudg, ///
      re vce(robust) level(90)
   ...
   graph export "figure_7_b.pdf", replace
restore
...
preserve
   ...
   graph export "figure_7_c.pdf", replace
restore
...
preserve
   ...
   collapse (count) n=subject ///
      (sum) m=dectype2, ///
      by(dectype scenario treatment case)

   label define casel_lb 1 "Fall 1" 2 "Fall 2" 3 "Fall 3" 4 "Fall 4" 5 "Fall 5"
      label values case sase_lb
   ...
   twoway (bar n dectype if treatment==0 & scenario==0, fcolor(gs10) lcolor(black) barwidth(0.4)) ///
      (bar n dectype if treatment==1 & scenario==0, fcolor(gs14) lcolor(black) barwidth(0.4)) ///
      ...
      xlabel(1.2 "Weniger" 2.2 "Gleichverteilung" 3.2 `""Partielle" "Kompensation""' 4.2 "Bedarfsanteil" 5.2 "Mehr", labsize(large) angle(v)) xtitle("Bedarfsszenario", size(large)) ///
      legend(order(3 "Nettoaufteilung" 1 "Geringe Verantw.") size(medlarge) region(lwidth(none)) col(2)) ///
      saving(figure_8_a, replace)

   twoway (bar n dectype if treatment==0 & scenario==1, fcolor(gs10) lcolor(black) barwidth(0.4)) ///
      (bar n dectype if treatment==1 & scenario==1, fcolor(gs14) lcolor(black) barwidth(0.4)) ///
      ...
      xlabel(1.2 "Weniger" 2.2 `""Produktivitäts-" "anteil""' 3.2 `""Partielle" "Kompensation""' 4.2 "Gleichverteilung" 5.2 "Mehr", labsize(large) angle(v)) ///
      legend(order(2 "Hohe Verantw." 3 "Tausch") size(medlarge) region(lwidth(none)) col(2)) ///
      saving(figure_8_b, replace)

   graph combine figure_8_a.gph figure_8_b.gph, col(2) graphregion(color(white)) xsize(4) altshrink ///
...
preserve
   ...
   collapse (sum) equald propd partd netd ///
      (first) treatment, ///
      by(subject scenario)
   ...
   twoway (bar n stype if treatment==0 & scenario==0, fcolor(gs10) lcolor(black) barwidth(0.4)) ///
      (bar n stype if treatment==1 & scenario==0, fcolor(gs14) lcolor(black) barwidth(0.4)), ///
      ytitle("Relative Häufigkeit", size(large)) ///
      yscale(range(0 0.4)) ///
      xscale(range(0.7 4.5)) ///
      xlabel(1.2 `""Gleich-" "verteilung""' 2.2 `""Partielle" "Kompensation""' 3.2 `""Bedarfs-" "anteil""' 4.2 `""Netto-" "aufteilung""', labsize(large) angle(v)) ///
   ...
   twoway (bar n stype if treatment==0 & scenario==1, fcolor(gs10) lcolor(black) barwidth(0.4)) ///
      (bar n stype if treatment==1 & scenario==1, fcolor(gs14) lcolor(black) barwidth(0.4)), ///
      ytitle("") ///
      yscale(range(0 0.4)) ///
      xlabel(1.2 `""Gleich-" "verteilung""' 2.2 `""Partielle" "Kompensation""' 3.2 `""Produktivitäts-" "anteil""' 4.2 "Tausch", labsize(large) angle(v)) ///
      xtitle("Produktivitätsszenario", size(large)) ///
      xscale(range(0.7 4.5)) ///
   ...
   graph combine figure_9_a.gph figure_9_b.gph, col(2) graphregion(color(white)) ycommon ///
...
cibar eval, over1(kind_of_need) graphopts(ytitle("Einschätzung") graphregion(color(white)))
...
cibar allocation_diff, over2(kind_of_need) over1(productivity) ///
   graphopts(ytitle("Durchschnittliche Differenz") xlabel(, labsize(small))) ///
...
cibar allocation_diff, over2(case) over1(productivity) ///
   graphopts(ytitle("Durchschnittliche Differenz") xlabel(, labsize(small) alternate)) ///
   baropts(lcolor(black) lpattern(solid) lwidth(medium) graphregion(color(white)))
